...
def crop(gens, seconds=5, cropper=None):
	...
	if cropper == None:
		print("cropping %d channel(s) to %d samples each" % (len(gens), seconds * FRAME_RATE))
		cropper = lambda gen: itertools.islice(gen, 0, seconds * FRAME_RATE)
...
def crop_with_fades(gen, seconds, fade_in=0.01, fade_out=0.01):
	...
	total_samples = int(seconds * FRAME_RATE)
	fade_in_samples = int(fade_in * FRAME_RATE)
	fade_out_samples = int(fade_out * FRAME_RATE)
...
def crop_with_fade_out(gen, seconds, fade=.01):
	...
	total_samples = int(seconds * FRAME_RATE)
	fade_samples = int(fade * FRAME_RATE)
...
def crop_at_zero_crossing(gen, seconds=5, error=0.1):
	...
	'''
	source = iter(gen)
	buffer_length = int(2 * error * FRAME_RATE)
	...
	start = itertools.islice(source, 0, int((seconds - error) * FRAME_RATE))
