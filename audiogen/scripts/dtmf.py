...
def main():
    ...
    parser.add_argument("-d", "--digits",
        ...
        type=str.upper,
        default=[],
        choices=["1", "2", "3", "4", "5", "6", "7", "8", "9", "0", "*", "#", "A", "B", "C", "D"],
        help="DTMF digits to playback. Should be one of [1234567890*#ABCD]. " +
             "May be provided multiple times."
    )
    parser.add_argument("frequencies", nargs='*', type=int,
        help="Frequency of tones to play. Should be either two or zero positional arguments. " +
            "If --digit options are also provided, the digit tones are mixed with the specified " +
            "frequencies. If neither --digit nor posititional arguments are " +
            "provided, defaults to a standard North American dial tone combining 350 Hz and 440 Hz."
        )
    args = parser.parse_args()

    frequencies = []

    if len(args.frequencies) == 0 and len(args.digits) == 0:
    ...
    frequencies += [ frequency for pair in digitFrequencyPairs for frequency in pair ]

    tones = [ audiogen.beep(frequency, args.seconds) for frequency in frequencies ]

    samples = audiogen.util.mixer(tones)
